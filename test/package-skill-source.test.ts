...
const writePackage = Effect.fn("writePackageSkillFixturePackage")(function* (
  project: string, packageName: string, version: string, skill: string, body = "Nested.",
) {
  ...
  yield* fs.writeFileString(path.join(root, "package.json"), JSON.stringify({
    name: packageName,
    version,
    intent: { version: 1, repo: `https://example.test/${packageName}`, docs: "https://example.test/docs" },
  }));
  yield* fs.writeFileString(path.join(root, "skills", skill, "SKILL.md"), `---\nname: ${skill}\ndescription: ${packageName} ${skill}.\n---\n`);
  yield* fs.writeFileString(path.join(root, "skills", skill, "nested", "note.md"), body);
  return root;
});

const fixture = Effect.fn("packageSkillFixture")(function* (dependencies: Readonly<Record<string, string>>) {
  const fs = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
...
describe("installed package skill discovery", () => {
  layer(NodeServices.layer)((it) => {
    it.effect("discovers scoped and unscoped direct dependencies, retaining nested files", () => Effect.gen(function* () {
      ...
      assert.deepEqual(found.candidates.map((skill) => skill.selector), ["@scope/skills#beta", "plain-skills#alpha"]);
      const exact = yield* resolvePackageSkillSelector(project, "plain-skills#alpha");
      assert.strictEqual(exact.version, "1.0.0");
      assert.isTrue(yield* (yield* FileSystem.FileSystem).exists((yield* Path.Path).join(exact.path, "nested", "note.md")));
    }));

    it.effect("does not browse installed packages that are not direct dependencies", () => Effect.gen(function* () {
      ...
      const found = yield* discoverPackageSkills(project);
      assert.deepEqual(found.candidates.map((skill) => skill.selector), ["direct-skills#alpha"]);
    }));
    ...
    it.effect("includes development, optional, and peer direct dependencies", () => Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const project = yield* fixture({});
      yield* fs.writeFileString(path.join(project, "package.json"), JSON.stringify({
      ...
      const found = yield* discoverPackageSkills(project);
      assert.deepEqual(found.candidates.map((skill) => skill.selector), ["development#dev", "optional#optional", "peer#peer"]);
    }));

    it.effect("uses stable node_modules link paths while canonicalizing pnpm-style package links", () => Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      ...
      assert.strictEqual(first.linkPath, path.join(project, "node_modules", "@scope", "skills", "skills", "alpha"));
      assert.strictEqual(first.path, path.join(yield* fs.realPath(store), "skills", "alpha"));
    }));

    it.effect("skips malformed packages in discovery and reports diagnostics, while exact selection fails", () => Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const project = yield* fixture({
        good: "1",
      ...
      yield* fs.writeFileString(path.join(broken, "skills", "beta", "SKILL.md"), "---\nname: nope\n---\n");
      const found = yield* discoverPackageSkills(project);
      assert.deepEqual(found.candidates.map((skill) => skill.selector), [
        "broken#valid",
        "good#alpha",
      ]);
      assert.deepEqual(found.diagnostics.map((diagnostic) => diagnostic.package), [
        "broken",
        "nodescription",
        "unmarked",
      ]);
    ...
    it.effect("rejects symlinked skill trees and keeps duplicate skill names distinct by selector", () => Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      ...
      const found = yield* discoverPackageSkills(project);
      assert.deepEqual(found.candidates.map((skill) => skill.selector), ["one#same", "two#same"]);
